...
void ARollingBallPawn::BeginPlay()
{
	Super::BeginPlay();
	...
	if (StaticMeshComponents.Num() > 0)
	{
		...
		StaticMeshComponent->OnComponentHit.AddDynamic(this, &ARollingBallPawn::OnHit);
	}
}

// Puts pawn in the starting state
void ARollingBallPawn::ResetToStartState()
{
...
void ARollingBallPawn::OnHit(UPrimitiveComponent* HitComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp,
...
TArray<ARollingBallEnemyActor*> ARollingBallPawn::GetEnemiesInRange(float Range)
{
	// Find all enemies actors
	TArray<AActor*> FoundActors;
	UGameplayStatics::GetAllActorsOfClass(GetWorld(), ARollingBallEnemyActor::StaticClass(), FoundActors);

	TArray<ARollingBallEnemyActor*> EnemiesInRange;

	// For every found actor 
	for (AActor* Actor : FoundActors)
	{
		// If actor is enemy
		if (ARollingBallEnemyActor* EnemyActor = Cast<ARollingBallEnemyActor>(Actor))
		{
...
void ARollingBallPawn::DestroyNextEnemy()
{
	...
	if (EnemiesImpactedByAbility.Num() > 0)
	{
		...
		GetWorld()->GetTimerManager().SetTimer(TimerHandle, this, &ARollingBallPawn::DestroyNextEnemy,
...
float ARollingBallPawn::CalculateAngleToEnemy(FVector EnemyCoordinates)
{
...
void ARollingBallPawn::IncreaseVelocity()
{
	// Stop pawn if moving
	PawnVelocity = 0.0f;
...
void ARollingBallPawn::AddVelocityImpulse()
{
...
void ARollingBallPawn::ChangeDirection(float Input)
{
	// Stop pawn if moving
	PawnVelocity = 0.0f;
...
void ARollingBallPawn::UseAbility()
{
	...
	FTimerHandle TimerHandle;
	GetWorld()->GetTimerManager().SetTimer(TimerHandle, this, &ARollingBallPawn::DestroyNextEnemy, EnemyKillInterval, false);
